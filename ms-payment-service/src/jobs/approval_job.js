...
const paymentResultQueue = process.env.PAYMENT_RESULT_QUEUE_NAME;
...
function scheduleApproval(transaction) {
  ...
  setTimeout(async () => {
    try {
      ...
      await publish(paymentResultQueue, message);

      console.log(`[Approved] [${transaction.transaction_id}] após ${(delay / 1000).toFixed(1)}s`);
      console.log(`[RabbitMQ] [${transaction.transaction_id}] Enviando mensagem na fila ${paymentResultQueue}`);
    } catch (err) {
